...
2. Импортировать [файл](/docs/postman/providers.postman.json) в json формате
...
```
{
  ...
  "order_id": "{{order_id}},
  "amount": "{{amount}}",
  "deposit_amount": "{{deposit_amount}}", // optional, only if a rate is available
  "status": "{{StatusEnum}}",
  "reason": "{{reason}}", // optional, only for returned tasks
  "card": "{{client card}}",
